...
def island_scan_range_run(seq, base, min_len, max_len, max_gaps_len, max_gaps, max_loop, of, length, all_flag, nocore_flag, min_perfect):
	''' '''
			...
			if list_island_i:
				if (list_island_i[0].get_strt_idx() - end_i - 1) > max_loop:
					if counter >= length:
						...
						sys.stderr.flush()
						if not all_flag:
							print_paths_score(graph_island, list_island, seq, of, length, min_perfect)
						else:
							print_paths(graph_island, list_island, seq, of, length)
						#end if
					#end if
					# Reset variables #
		...
		for line in fi:
			if line.startswith('>') and first_header:
				...
				if not checked_min_bases:
					island_scan_range_run(seq, base, 2, max_len, max_gaps_len, max_gaps, max_loop, fo, island_num, args['allresult'], args['nocore'], min_perfect)
				else:
					island_scan_range_run(seq, base, min_len, max_len, max_gaps_len, max_gaps, max_loop, fo, island_num, args['allresult'], args['nocore'], min_perfect)
				#end if
